		...
		if ($sourcefileFullName -match ".*\\([0-9]{8})(.*).txt")
			{
				...
				$customerHotfixDateString = $customerHotfixDate.ToString().Substring(0,10)
				$customerHotfixInstallDateString = $sourcefile.LastWriteTime.ToString()
				...
				try{
					$InputObject.Add([pscustomobject]@{
						...
						Metrics    = @{CustomerHotfixName=$customerHotfixName;CustomerHotfixInstallationDate=$customerHotfixInstallDateString;CustomerHotfixDate=$customerHotfixDateString;Server=$servername;TimeToInstall=$customerHotfixTimeToFixDouble}
